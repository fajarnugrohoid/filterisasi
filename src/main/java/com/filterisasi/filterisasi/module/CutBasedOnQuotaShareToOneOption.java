...
public class CutBasedOnQuotaShareToOneOption {
    ...
    public CutBasedOnQuotaShareToOneOption(FindData findData, StudentHistory studentHistory, CheckQuotaBalance checkQuotaBalance, UpdateData updateData) {
    ...
    //berbagi kuota, tpi satu lawan satu
                ...
                if (ppdbOptions.get(iOpt).getPpdbRegistrationList().size() > ppdbOptions.get(iOpt).getQuota()) { //jika pendaftar lebih banyak dari quota, maka potong
                    ...
                    for (int iStd = students.size()-1; iStd>ppdbOptions.get(iOpt).getQuota()-1 ; iStd--) {
                        ...
                        if (idxHistoryTargetOption==-1){
                            ...
                            int idxThirdOption = findData.findIdxFromOptionsByChoiceId(students.get(iStd).getThirdChoice(),ppdbOptions);
                            this.studentHistory.addNewOrUpdateStudentHistory(ppdbOptions, students, iStd, acceptedOptionId, acceptedOptionNo, idxThirdOption);
